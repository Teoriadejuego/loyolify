...
create_conflicto_cohesion <- function(df){
  ...
  dfsoc[variable == 'cohesión' &  n >0 & n<quantile(n, c(0.50),na.rm=T), y:= 1]
  dfsoc[variable == 'cohesión' &  n >=quantile(n, c(0.50),na.rm=T) & n<quantile(n, c(0.80),na.rm=T), y:= 2]
  dfsoc[variable == 'cohesión' &  n >=quantile(n, c(0.80),na.rm=T) , y:= 3]
  
  dfsoc[variable == 'conflicto' &  n ==0, y:= 0]
  dfsoc[variable == 'conflicto' &  n>0 & n<quantile(n, c(0.50),na.rm=T), y:= 1]
  dfsoc[variable == 'conflicto' &  n >=quantile(n, c(0.50),na.rm=T) & n<quantile(n, c(0.80),na.rm=T), y:= 2]
  dfsoc[variable == 'conflicto' &  n >=quantile(n, c(0.80),na.rm=T), y:= 3]
